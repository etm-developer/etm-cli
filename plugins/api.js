...
function sendMoney(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = etmJS.transaction.createTransaction(
    options.to,
...
function sendAsset(options) {
  var trs = etmJS.uia.createTransfer(
    options.currency,
    options.amount,
    options.to,
    options.message,
...
function registerDelegate(options) {
  ...
  //   console.log(err || result);
  // });
  var trs = etmJS.delegate.createDelegate(
    options.username,
    options.secret,
    options.secondSecret
  );
...
function setSecondSecret(options) {
  var trs = etmJS.signature.createSignature(options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId)
  });
}
...
function registerDapp(options) {
  ...
  var trs = etmJS.dapp.createDApp(dapp, options.secret, options.secondSecret);
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId)
  });
}
...
function deposit(options) {
  var trs = etmJS.transfer.createInTransfer(options.dapp, options.currency, options.amount, options.secret, options.secondSecret)
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId)
  });
}
...
function dappTransaction(options) {
  var trs = etmJS.dapp.createInnerTransaction({
    fee: options.fee,
...
function lock(options) {
  var trs = etmJS.transaction.createLock(options.height, options.secret, options.secondSecret)
  getApi().broadcastTransaction(trs, function (err, result) {
    console.log(err || result.transactionId)
  });
}
...
function getTransactionBytes(options) {
  ...
  console.log(etmJS.crypto.getBytes(trs, true, true).toString('hex'))
}
...
function getTransactionId(options) {
  ...
  console.log(etmJS.crypto.getId(trs))
}
...
function getBlockPayloadHash(options) {
  ...
  for (let i = 0; i < block.transactions.length; ++i) {
    payloadHash.update(etmJS.crypto.getBytes(block.transactions[i]))
  }
...
function verifyBytes(options) {
  console.log(etmJS.crypto.verifyBytes(options.bytes, options.signature, options.publicKey))
}
